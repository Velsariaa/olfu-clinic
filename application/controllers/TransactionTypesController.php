...
class TransactionTypesController extends CI_Controller 
{
	private $parent_route = 'transaction-types';

	private $templates_dir = 'transaction_types';

	function __construct()
	{
		parent::__construct();

		$this->table_model = TransactionTypeModel::class;
	...
	public function delete($id)
	{
		...
		$transactions = TransactionModel::where('transaction_type_id', $this->table_row->id)
		->count();

		if ($transactions) json_response(false, ['message' => 'Transaction type is currently in use.']);
	...
	{
		$this->table_row->transaction_type = post('transaction_type', 'trim');
	}
	private function validate_table_row_fields()
	{
		$this->load->library('form_validation');

		$this->form_validation->set_rules('transaction_type', 'transaction type', [
			'required',
			TransactionTypeModel::is_unique('transaction_type', post('id'))
		]);
